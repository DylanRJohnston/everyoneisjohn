...
Meteor.methods({
    ...
    useWillPower: function(gameId, playerId, amount) {
        ...
        if (Meteor.isServer) {
            ...
            if (diceRoll + totalBonus >= game.checkDifficulty) {
                ...
                Games.update({_id: game._id}, {$set: {johnId: "", gameState: "challange", checkDifficulty: 0}});
            }
        }
    },
    betWillPower: function(gameId, playerId, willpower) {
        ...
        let finishedPlayers = Players.find({gameId: gameId, bet: {$gte: 0}}, {sort: {bet: -1}});
        ...
        Players.update({gameId: gameId}, {$set: {bet: -1}}, {multi: true});
